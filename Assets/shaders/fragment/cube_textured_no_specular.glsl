...
void main() {
    ...
    for(int i = 0; i <= active_dirlight_qty; i++) {
        result += calc_directional_light(dirlight[i], diffuse_material, scenelight, normalized_frag_cube_normals, view_direction, fragment_tex_coords);
    }

    // Point (active pointlight qty set to -1 during shader compilation)
    for(int i = 0; i <= active_pointlight_qty; i++){
        result += calc_point_light(pointlight[i], diffuse_material, scenelight, normalized_frag_cube_normals, view_direction, fragment_tex_coords, fragment_world_position);
    }
