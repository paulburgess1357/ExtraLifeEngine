...
void main(){	
    ...
    // Determine if vertex is front facing or back facing based on the 
    // dot product between a vector pointing towards the camera and the normals
    // If the dot product is negative, we know that the normals are pointing
    // away from the viewer (so the viewer is looking at the back face)
    float vec_to_cam_to_normals_dot = dot(-vertex_position_eye.xyz, vertex_normals_eye);
    
    // Output
    if(vec_to_cam_to_normals_dot > 0){    
