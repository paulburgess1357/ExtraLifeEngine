...
void OpenGL::OpenGLCubeMapCompiler::generate_textures(const bool apply_gamma_correction) const{
	...
	for (unsigned int i = 0; i < m_texture_loading_data.size(); i++) {		
		...
		const GLenum internal_format = OpenGL::OpenGLTextureFormatFinder::get_texture_internal_format(texture_loading_data.m_components);
		const GLenum format = OpenGL::OpenGLTextureFormatFinder::get_texture_standard_format(texture_loading_data.m_components);
