...
namespace ImGui
{
    ...
    IMGUI_API void          NewFrame();                                 // start a new Dear ImGui frame, you can submit any command from this point until RenderComponent()/EndFrame().
    IMGUI_API void          EndFrame();                                 // ends the Dear ImGui frame. automatically called by RenderComponent(). If you don't need to render data (skipping rendering) you may call EndFrame() without RenderComponent()... but you'll have wasted CPU already! If you don't need to render, better to not create any windows and not call NewFrame() at all!
    ...
    IMGUI_API ImDrawData*   GetDrawData();                              // valid after RenderComponent() and until the next call to NewFrame(). this is what you have to render.
    ...
    IMGUI_API ImGuiMouseCursor GetMouseCursor();                                                // get desired cursor type, reset in ImGui::NewFrame(), this is updated during the frame. valid before RenderComponent(). If you use software rendering by setting io.MouseDrawCursor ImGui will render those for you
...
struct ImGuiIO
{
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    // [OBSOLETE since 1.60+] Rendering function, will be automatically called in RenderComponent(). Please call your rendering function yourself now!
    // You can obtain the ImDrawData* by calling ImGui::GetDrawData() after RenderComponent(). See example applications if you are unsure of how to implement this.
    ...
    //------------------------------------------------------------------
    // Output - Updated by NewFrame() or EndFrame()/RenderComponent()
    ...
    int         MetricsRenderVertices;          // Vertices output during last call to RenderComponent()
    int         MetricsRenderIndices;           // Indices output during last call to RenderComponent() = number of triangles * 3
...
struct ImDrawList
{
    ...
    IMGUI_API void  PushClipRect(ImVec2 clip_rect_min, ImVec2 clip_rect_max, bool intersect_with_current_clip_rect = false);  // RenderComponent-level scissoring. This is passed down to your render function but not used for CPU-side coarse clipping. Prefer using higher-level ImGui::PushClipRect() to affect logic (hit-testing and widget culling)
...
struct ImDrawData
{
    bool            Valid;                  // Only valid after RenderComponent() is called and before the next NewFrame() is called.
