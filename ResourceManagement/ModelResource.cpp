...
std::shared_ptr<IModel> ModelResource::load(const std::string& model_name, const std::string& model_path, const std::string& model_shader_name){
	if(!is_loaded(model_name)){		
		Print::print("\nLoading Model: " + model_name + " (" + model_path + ")");
		load_model(model_name, model_path, model_shader_name);
	}
...
void ModelResource::load_model(const std::string& model_name, const std::string& model_path, const std::string& model_shader_name) {

	if (GraphicsAPI::get_api() == GraphicsAPIType::OPENGL) {
		load_opengl_model(model_name, model_path, model_shader_name);
		return;
	}

	if (GraphicsAPI::get_api() == GraphicsAPIType::VULKAN) {
		load_vulkan_model(model_name, model_path, model_shader_name);
		return;
	}
...
void ModelResource::load_opengl_model(const std::string& model_name, const std::string& model_path, const std::string& model_shader_name) {
	OpenGL::OpenGLModelLoaderFromFile model_loader{ model_path };
	const std::shared_ptr<IShaderProgram> model_shader_program = ShaderResource::get(model_shader_name);
	m_model_cache[model_name] = std::make_shared<OpenGL::OpenGLModel>(model_loader, model_shader_program);
}

void ModelResource::load_vulkan_model(const std::string& model_name, const std::string& model_path, const std::string& model_shader_name) {
